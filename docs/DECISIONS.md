...
Security hardening at this boundary is fail-closed. Lensfun camera make/model must select exactly one database entry before GekiGrade trusts its mounts; duplicate normalized records produce no correction-support claim. Doctor rejects a symlinked RawTherapee CLI before consulting bundle metadata and requires the selected output ICC bytes to remain identity-stable and parse through LittleCMS. Source validation before/after development and at publication opens without following symlinks and without blocking on special files, then requires a stable regular-file identity and the recorded digest; a failed post-run check records the failure and removes developed output, while every failed post-publication check removes the provisional manifest.
