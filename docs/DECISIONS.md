...
External-tool readiness and metadata must reflect the same controlled execution used for processing. ExifTool is invoked with external configuration disabled and a minimal recorded C-locale environment for both version and metadata reads. The doctor ImageMagick probe uses the same allowlisted environment as normalization and preview generation rather than inheriting caller module/configuration paths.

Security hardening at this boundary is fail-closed. Lensfun camera make/model must select exactly one database entry before GekiGrade trusts its mounts; duplicate normalized records produce no correction-support claim. Doctor rejects a symlink in the RawTherapee CLI or any path ancestor before consulting bundle metadata and requires the selected output ICC bytes to remain identity-stable and identify an RGB display-class profile through LittleCMS. Source signature and hash validation around metadata extraction, before/after development, and at publication opens without following symlinks and without blocking on special files, then requires a stable regular-file identity and the recorded digest; a failed post-run check records the failure and removes developed output, while every failed post-publication check removes the provisional manifest.
