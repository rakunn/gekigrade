...
## Selected for the future RAW adapter, not Milestone 1 rendering

RawTherapee 5.13 is installed from `brew install --cask rawtherapee`. The future adapter will call `rawtherapee-cli` with an explicit PP3 and request a 16-bit TIFF, capture stderr, record the application/PP3/profile hashes, and report applied lens capabilities. RawTherapee is GPL-3.0; using a separately installed executable is suitable for this personal tool, while bundling or redistribution would require a dedicated GPL compliance review. Current local installation and PP3 reproducibility are confirmed only at the executable/design level; actual ARW output, camera color, demosaicing, lens correction, and chromatic-aberration behavior remain unverified.
