...
- Source format detection reads through a non-blocking, no-follow, identity-stable regular-file descriptor. RAW hashes immediately before and after ExifTool use the same contract, so special-file or symlink replacement fails inspection; the copied PP3 hash must remain unchanged across RAW execution.
