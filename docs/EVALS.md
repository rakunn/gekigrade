...
- Static RAW output-profile, camera-resource, and Lensfun failures occur before job-directory creation, leaving the requested output path available for a corrected retry.
...
- The prepared manifest uses the RawTherapee tool status accepted by the successful development boundary. Changing executable permissions or `Info.plist` after development must not make manifest readiness describe a different tool snapshot.
...
- Any symlink elsewhere in the selected RawTherapee application bundle is also rejected before job creation, matching doctor readiness and runtime snapshot policy.
...
- RAW doctor readiness requires a symlink-free selected application bundle, present camera-profile directories, a regular non-symlink output profile that LittleCMS parses as RGB display-class ICC, a parseable alias map, structurally valid RawTherapee JSON-with-comments camera constants, and a complete parseable Lensfun XML set.
...
- Preview encoding reads an identity-bound, read-only snapshot of the validated working TIFF. Replacing and restoring the retained working path during encoding cannot change preview pixels; both snapshot and retained artifact remain hash-stable through publication. The preview must decode as three-channel JPEG, embed the expected sRGB profile, and remain hash-stable through pixel loading and manifest publication.
...
- JSON publication uses an unpredictable, uniquely and exclusively created temporary file in the destination directory before atomic replacement. An abandoned fixed-name file or symlink cannot block a later update or overwrite its target, and temporary files from ordinary exceptions are removed.
- `source.json` must equal the canonical digest of the accepted in-memory provenance before and after manifest publication. Substitution during artifact scanning removes the provisional manifest and fails preparation.
