...
Do not infer technical measurements that are already present in QA. Do not select a candidate that has fatal QA. If all candidates are worse than the source, say so and revise the plan within the existing schema. Otherwise run `geki select JOB ID` with an exact candidate ID.
